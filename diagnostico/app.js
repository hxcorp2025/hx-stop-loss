/* ============================================================
   HX · /diagnostico/ — Teto de Aquisição (frame ofensivo)
   motion + carrossel de headlines + atribuição + formulário MQL
   ============================================================ */
(function () {
  'use strict';
  ...
  ['.stat-row .cell', '.quotes .quote', '.step', '.logos span', '.hero-trust span'].forEach(function (sel) {
  ...
  /* ---------- carrossel de headlines (modo apresentação) ----------
     #1 = a variação que o Matheus gostou (Proclamação, da /aplicacao/).
     As demais são o frame ofensivo do Teto de Aquisição.
     Remover este bloco (e os controles .hv-*) antes do tráfego. */
  var VARIANTES = [
    { id: '1 · Proclamação', html: 'A maioria das operações digitais desperdiça, todo mês, <em>mais lucro do que gastaria pra consertar o problema de vez.</em>' },
    { id: '2 · Mecanismo', html: 'O que decide quem escala não é o melhor criativo. É <em>quanto você pode pagar por um cliente e ainda lucrar.</em>' },
    { id: '3 · Inimigo', html: 'Seu concorrente não vende mais porque é melhor que você. Ele vende mais porque <em>pode pagar mais caro por cada cliente.</em>' },
    { id: '4 · Domínio', html: 'Quem pode pagar mais pra ganhar um cliente domina o leilão. A HX <em>levanta o teto que trava a sua escala hoje.</em>' },
    { id: '5 · Segredo', html: 'Existe um número que decide o quanto você consegue escalar. <em>A maioria dos donos nunca calculou o dele.</em>' },
    { id: '6 · Desejo', html: 'Você não precisa de mais tráfego. Precisa <em>poder gastar mais que o concorrente sem perder dinheiro.</em>' }
  ];
  ...
  if (h1 && label && prev && next) {
    var idx = 0, busy = false;
    var renderH = function (k) {
      idx = (k + VARIANTES.length) % VARIANTES.length;
      h1.innerHTML = VARIANTES[idx].html;
      label.innerHTML = 'Lead <b>' + VARIANTES[idx].id + '</b> · ' + (idx + 1) + '/' + VARIANTES.length;
    };
    var go = function (d) {
  ...
  var CFG = { FORM_ENDPOINT: '', TTL: 90, KEY: 'hx_diagnostico_attrib' };
  ...
  form.addEventListener('submit', function (e) {
    ...
    ['faturamento', 'investimento_aquisicao'].forEach(function (k) {
    ...
    fetch(CFG.FORM_ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      .then(function (res) {
        ...
        btnEnviar.textContent = 'Diagnóstico solicitado';
        setFeedback('Recebemos seu pedido. Um sócio da HX responde com prioridade.', true);
        if (typeof fbq === 'function') fbq('track', 'Lead');
        if (window.dataLayer) window.dataLayer.push({ event: 'lead_diagnostico' });
      })
      .catch(function () { btnEnviar.disabled = false; btnEnviar.textContent = 'Quero o diagnóstico'; setFeedback('Deu ruim no envio. Tenta de novo em um minuto.', false); });
  });
})();
